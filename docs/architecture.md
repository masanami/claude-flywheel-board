...
- **カードに置く操作は 2 つだけ**: ⚠更新なしカード（と詳細モーダル）の「再開コマンドを挿入」＝プリフィル（FR-12）と、**承認待ちカードの承認ボタン**（FR-20・#165）。承認は誤操作対策として 2 段階（押す → 確認 → 承認）にする。**エージェントの状態機械を変えるボタン（ステータス変更・実行・却下）は引き続き置かない**（NFR-01 区分③）。
...
- **UI 動線**:
  ...
  - 承認: 承認待ちカードの承認ボタンから board が直接書く（FR-20・#165。プリフィルではない唯一のカード操作）。該当タブの対話で伝える従来の動線も引き続き有効。
...
2. カードの承認ボタンを押し、確認して承認する（FR-20）。`POST /api/challenges/:agent/:id/approve`。
