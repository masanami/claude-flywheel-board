...
board 左サイドの開閉式パネルに、エージェント repo 配下の Markdown ファイルを読み取り専用でレンダリング表示する。fs-watch 連動で「nvim で保存 → 即時再レンダリング」のライブプレビューを提供する。
...
fleet の管理者として、nvim でドキュメントを編集しながら board の左サイドパネルでレンダリング結果をリアルタイムに確認したい。また、エージェントの docs や台帳を board 上でレンダリングされた状態で読みたい。

## 機能要件

- [ ] **左サイドパネル**（#112 で右から移動）: board 画面左側の開閉式パネルとしてプレビューを表示する。ボード・下部ターミナル領域とは同時表示（ターミナル領域を閉じない・高さに影響しない）。パネル幅はドラッグで調整できる
- [ ] **簡易ファイルツリー**: manifest 登録済みの各エージェント repo 配下の `.md` ファイルをツリー表示し、クリックで開く。`.` 始まりの全ディレクトリ（`.git` を含む）と `node_modules` は走査から除外する（除外ルールはこの機械的判定のみ。明示リストや .gitignore 解釈は行わない）。ツリー走査はシンボリックリンクを辿らない（lstat で判定し、ディレクトリの symlink は除外する。循環参照による無限ループ防止）。ツリーはパネルを開いたとき、およびパネルヘッダのリフレッシュボタン押下時に取得する（常時監視しない）。
  実装ノート（#68 → #110 で更新）: `GET /api/md/tree` は `files` を repo 相対パス文字列の配列（ディレクトリ階層を持たないフラットな形）で返す（この API 契約は #110 でも不変）。UI 側は #110 で IDE 風のネストツリー表示に変更した: エージェント（repo）を最上段の開閉ノード（初期: 展開）、ディレクトリを開閉ノード（初期: 折りたたみ）とし、ファイルはベース名のみ表示する。ネスト構造はクライアント側（FileTree.tsx）でフラットなパス一覧から組み立てる表示専用の導出であり、サーバはツリー構造を持たない。開閉状態はリフレッシュ後も保持し、パネルを閉じると破棄する（選択状態と同じライフサイクル）
...
- **UI レイアウト**: 左サイドパネルは flex でボードカラム領域を圧縮して確保する（#112 でボードカラムの前に配置）。下部ターミナル領域の高さ・幅には影響しない
...
- [ ] manifest 登録 repo の `.md` がファイルツリーに表示され、クリックすると左サイドパネルにレンダリング表示される
