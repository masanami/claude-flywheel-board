...
function createFakeWebSocket(): {
  ...
  /** テストから ws.bufferedAmount を直接操作するためのセッター（バックプレッシャー制御。Issue #26）。 */
  setBufferedAmount: (value: number) => void;
  /** クライアント側切断による "close" イベントの発生を模擬する（fake 自身の close() とは別。あちらは production コードが ws.close() を呼ぶケース用）。 */
  emitClose: () => void;
...
describe("createTerminalWebSocketServer（純粋なユニット部分）", () => {
  ...
  it("kind クエリが 'agent'/'shell' 以外の不正値の場合は destroy する（#57）", () => {
    const tmux = createFakeTmux();
    const { ptyProcess } = createFakePty();
    const bridge = createTerminalWebSocketServer({
      getFleetEntries: () => FLEET_ENTRIES,
    ...
    const write = vi.fn();
    const request = {
      url: "/ws/terminal?agent=medical&kind=bogus",
      headers: { host: "localhost" },
    } as never;
    const socket = { destroy, write } as never;
...
describe("createTerminalWebSocketServer（実ソケットでの結合テスト。tmux/pty は Mock）", () => {
  ...
  it("kind=shell で接続すると -shell サフィックス付きセッション名で has-session/new-session/pty spawn される（#57）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const spawnPty = vi.fn().mockReturnValue(fakePty.ptyProcess);
    const bridge = createTerminalWebSocketServer({
    ...
    const port = await startServer(bridge);
    const ws = new WebSocket(
      `ws://127.0.0.1:${port}/ws/terminal?agent=medical&kind=shell`,
      {
        headers: { origin: "http://localhost:5173" },
      },
    );
    ...
    expect(tmux.hasSession).toHaveBeenCalledWith("flywheel-medical-shell");
    expect(tmux.newSession).toHaveBeenCalledWith(
  ...
  it("【安全要件・#57】kind=shell 接続では prefill メッセージが tmux.sendKeysLiteral を呼ばない（構造的に無視される）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const bridge = createTerminalWebSocketServer({
      getFleetEntries: () => FLEET_ENTRIES,
    ...
    ws.send(JSON.stringify({ type: "prefill", command: "git status" }));
    // prefill には「実行された」ことを示す正のシグナルが無い（安全要件により
    // 何も起きないはずのため）。同一 WS 接続内でメッセージは到着順に処理される
    // ため、prefill 送信の直後に送った別種メッセージ（input）が処理されるのを
    // 待つことで、「prefill の処理（無視されるはず）は既に完了済み」であることを
    // 保証してから sendKeysLiteral 未呼び出しを検証する。
    ws.send(
      JSON.stringify({ type: "input", data: "marker-after-shell-prefill" }),
    );
    await vi.waitFor(() => {
      expect(fakePty.write).toHaveBeenCalledWith("marker-after-shell-prefill");
    });

    expect(tmux.sendKeysLiteral).not.toHaveBeenCalled();
...
describe("createTerminalWebSocketServer（バックプレッシャー制御。Issue #26 / PR #24 CodeRabbit 指摘）", () => {
  /**
   * pty 起動（onData 購読開始）を fake WS 経由で待つ。実ソケットを使わず、
   * cache.test.ts の setIntervalFn DI パターンと同様に、タイマーの発火も
   * テストから明示的に駆動できるようにする。
   */
  async function readyFakePty(
    emitMessage: (raw: string) => void,
    fakePty: ReturnType<typeof createFakePty>,
  ): Promise<void> {
    emitMessage(JSON.stringify({ type: "input", data: "" }));
    await vi.waitFor(() => {
      expect(fakePty.write).toHaveBeenCalled();
    });
  }

  it("ws.bufferedAmount が高水位を超えている状態で pty からデータが届いたら ptyProcess.pause() が呼ばれる", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    ...
    const fake = connectFakeWebSocket(bridge);
    await readyFakePty(fake.emitMessage, fakePty);

    // 高水位（1MB）を超えるバッファ量を模擬する。
    fake.setBufferedAmount(2_000_000);
    fakePty.emitData("large chunk from a runaway process");

    expect(fakePty.pause).toHaveBeenCalledTimes(1);
    expect(fakePty.resume).not.toHaveBeenCalled();
  });

  it("高水位超過が続く間、pause() は多重発火しない（冪等な呼び出し）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const setIntervalFn = vi.fn(() => 1 as unknown as NodeJS.Timeout);
    const clearIntervalFn = vi.fn();
    ...
    fake.setBufferedAmount(2_000_000);
    fakePty.emitData("chunk 1");
    fakePty.emitData("chunk 2");
    fakePty.emitData("chunk 3");

    expect(fakePty.pause).toHaveBeenCalledTimes(1);
  });

  it("pause 後、ws.bufferedAmount が低水位を下回ったら ptyProcess.resume() が呼ばれる（タイマー駆動）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    ...
    fake.setBufferedAmount(2_000_000);
    fakePty.emitData("large chunk");
    expect(fakePty.pause).toHaveBeenCalledTimes(1);
    // pause 中はポーリング用タイマーが起動している。
    expect(setIntervalFn).toHaveBeenCalledTimes(1);

    // バッファが低水位（256KB）を下回るまで捌けた状態を模擬する。
    fake.setBufferedAmount(100_000);
    scheduled?.();

    expect(fakePty.resume).toHaveBeenCalledTimes(1);
    // resume したポーリング用タイマーは片付けられる。
    expect(clearIntervalFn).toHaveBeenCalledTimes(1);
  });

  it("低水位を下回っていない間はタイマーが発火しても resume() は呼ばれない", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    let scheduled: (() => void) | undefined;
    const setIntervalFn = vi.fn((handler: () => void) => {
    ...
    // まだ低水位（256KB）を超えたまま。
    fake.setBufferedAmount(500_000);
    scheduled?.();
  ...
  it("WS 切断（close）時、pause 中のポーリング用タイマーが後始末される（リークしない）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const setIntervalFn = vi.fn(() => 1 as unknown as NodeJS.Timeout);
    const clearIntervalFn = vi.fn();
  ...
  it("WS 切断（error）時、pause 中のポーリング用タイマーが後始末される（リークしない）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const setIntervalFn = vi.fn(() => 1 as unknown as NodeJS.Timeout);
    const clearIntervalFn = vi.fn();
    ...
    fake.emitError(new Error("socket error"));

    expect(clearIntervalFn).toHaveBeenCalledTimes(1);
  });

  it("resume 後、再び高水位を超えたら pause が再発火する（一度きりで固定化しない）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    let scheduled: (() => void) | undefined;
    const setIntervalFn = vi.fn((handler: () => void) => {
    ...
    // 1 回目の pause → resume サイクル。
    fake.setBufferedAmount(2_000_000);
    fakePty.emitData("large chunk 1");
    expect(fakePty.pause).toHaveBeenCalledTimes(1);
    ...
    // 2 回目の高水位超過。resume 後も pause が再度発火することを確認する。
    fake.setBufferedAmount(2_000_000);
    fakePty.emitData("large chunk 2");

    expect(fakePty.pause).toHaveBeenCalledTimes(2);
    // タイマーも都度張り直される（1 回目の pause で1回 → 2回目の pause でもう1回）。
    expect(setIntervalFn).toHaveBeenCalledTimes(2);
  });

  it("pause 中に pty が終了（onExit）したら、ポーリング用タイマーが後始末される", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const setIntervalFn = vi.fn(() => 1 as unknown as NodeJS.Timeout);
    const clearIntervalFn = vi.fn();
  ...
  it("bufferedAmount が低い通常時は、既存の「pty出力がWSに届く」動作を壊さない（pause/resume は呼ばれない）", async () => {
    const tmux = createFakeTmux();
    const fakePty = createFakePty();
    const bridge = createTerminalWebSocketServer({
      getFleetEntries: () => FLEET_ENTRIES,
    ...
    expect(fake.send).toHaveBeenCalledWith("hello from pty\r\n");
    expect(fakePty.pause).not.toHaveBeenCalled();
    expect(fakePty.resume).not.toHaveBeenCalled();
  });
});
