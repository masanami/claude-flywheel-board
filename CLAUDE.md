...
- 機能仕様は P1〜P3 とも定義済み（ドキュメントマップ参照）。P1 は実装進行中（親 Issue #1・統合ブランチ feat/issue-1）。
- **P3（実行中パネル）のブロッカーは解消済み**: claude-flywheel PR #45（2026-07-16）で runs.jsonl 仕様が確定（正本: claude-flywheel `templates/runtime/README.md`）。残る依存は P2 のみ。
